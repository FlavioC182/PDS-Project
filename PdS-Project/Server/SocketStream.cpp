...
void SocketStream::waitingForConnection() {
	...
	/* La funzione accept accetta una connessione in entrata su un certo socket.
	*  Se al momento della chiamata non c'è nessuno in attesa la funzione blocca fino a che un client non si connette.
	*  I suoi parametri sono: 
	*  - SOCKET s: Identificatore di un socket messo in stato di Listen tramite la funzione listen.
	*  - sockaddr* addr: Indirizzo dell'entità che vuole connettersi (è opzionale, può essere inviato anche un parametro nullo, come effettivamente avviene nel nostro caso).
	*  - int* addrlen: Lunghezza dell'indirizzo dell'entità che vuole connettersi.
	*
	*  Il valore di ritorno dalla funzione accept è un socket che identifica il client, con il quale è possibile inviare e ricevere dati
	*  Nel nostro caso, la accept accetta la connessione da parte del client sul socket del server che era messo in stato di Listen.
	*/
	...
	/* A questo punto la connessione è stata accettata dal ListenSocket, pertanto possiamo affermare che il client è connesso
	*  Impostiamo lo stato del socket come connesso, per indicare che c'è una connessione attiva su di esso
	*/
